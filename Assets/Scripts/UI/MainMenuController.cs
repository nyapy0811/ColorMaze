...
/// chapterScenes에서 씬 이름을 찾아 로드한다(씬 이름은 Build Settings에 등록된 이름과 같아야 함).
...
public class MainMenuController : MonoBehaviour
{
    ...
    [Header("챕터별 스테이지 씬 이름 (인덱스 = 챕터 버튼에 연결한 번호)")]
    [SerializeField] ChapterStageScenes[] chapterScenes;
    ...
    /// <summary>챕터 버튼 OnClick에 챕터 번호(0부터, chapterScenes의 인덱스)를 인자로 연결한다.
    ...
    /// 현재 선택된 챕터(currentChapterIndex) 기준으로 chapterScenes에서 씬 이름을 찾아 로드한다.</summary>
    public void OnStageButton(int stageIndex)
    {
        if (currentChapterIndex < 0 || currentChapterIndex >= chapterScenes.Length) return;
        var scenes = chapterScenes[currentChapterIndex].sceneNames;
