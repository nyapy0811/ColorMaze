...
- [x] **스택→색 변환 공식 수정** (3.3) — `ColorStacks.ToRGB`가 기존에는 R/G/B 셋 중 최댓값을 기준으로 정규화해서(비율 변환), 예를 들어 (10,10,10)도 흰색으로 보이는 문제가 있었음. 채널마다 독립적으로 절대 스택 값을 [0, 255]로 매핑하도록 변경 — 이제 세 채널이 모두 상한값일 때만 흰색이 되고, 나머지는 실제 스택 값에 비례한 밝기로 표시된다. 이 함수를 쓰는 모든 기물(팔레트/필터/캔버스/컬러 체인저/뷰모델 붓)의 색 표시에 전부 반영됨.
- [x] **스택 상한값 31 → 15로 변경** (3.3) — `Config.max` 기본값과 `ColorStacks.ToRGB`의 `StackMax` 상수를 15로 변경. 스택 값 범위가 [0, 15]로 바뀌었고(15/15/15가 흰색), 순환·오버플로우 로직은 `max` 값을 그대로 참조하므로 별도 수정 없이 자동으로 적용됨. → 이후 Player 프리팹의 `ColorStacks` 컴포넌트에 R/G/B `max`가 각각 인스펙터에 10으로 저장돼 있던 게 원인이 되어 오버플로우가 10에서 일어나는 버그 발견(코드 기본값을 바꿔도 이미 저장된 인스펙터 값은 안 바뀜) → 아래 항목으로 근본 수정.
- [x] **튜토리얼 안내판 텍스트 (기획서 미기재, 신규)** — 위치·방향 모두 고정된 정적 텍스트라 스크립트가 필요 없음. 씬에 3D TextMeshPro 오브젝트를 원하는 위치·회전으로 배치하고 튜토리얼 문구를 직접 입력하면 끝. (처음엔 카메라를 바라보는 빌보드로 구현했다가, 방향도 고정해달라는 요청에 따라 불필요해진 `MapObjects/BillboardText.cs`는 내용을 비우고 `!!!!` 접두사를 붙여둠 — 삭제 권한 문제로 직접 못 지움, Unity에서 파일+`.meta` 삭제 필요.)
- [x] **블록 편집 모드 미리보기 큐브, 가려지는 모서리 안 보이게 수정** — `MazeGeneratorEditor.OnSceneGUI()`의 설치/제거 미리보기 와이어큐브가 `Handles.zTest = CompareFunction.Always`라 다른 블록에 가려져도 항상 그려졌음. `CompareFunction.LessEqual`(일반 깊이 테스트)로 변경해 가려지는 부분은 정상적으로 안 보이게 함.
- [x] **RGB 필터 통과 조건 강화** (4.3) — 기존엔 지정 색이 최댓값 채널 목록(동률 포함)에 있으면 통과였으나, 이제 그 색이 **동률 없이 유일한 최댓값**일 때만 통과하도록 변경. `RgbFilterBlock.Matches()`가 `GetMaxColors().Count == 1 && [0] == targetColor`로 판정.
- [x] **필터 테두리 머티리얼 지정 가능하게 변경** (4.2, 4.3) — `FilterBlockBase`에 `borderMaterial` 필드 추가. 지정하면 그 머티리얼을 그대로 쓰고, 비워두면 기존처럼 기본 아웃라인 셰이더(`Custom/FilterBorderAlwaysVisible`)로 자동 생성한 공용 머티리얼을 씀(그룹 설정은 여전히 그룹 내 첫 블록 기준).
- [x] **컬러 필터 라벨 형식을 ByRatio → ByValue로 변경** (3.3, 4.2) — `ToRGB`가 더 이상 비율(상대 최댓값) 정규화가 아니라 채널별 절대값 변환이라, 필터의 색 비교(`Matches`)가 사실상 정숫값 비교와 동치가 됨. 그에 맞춰 `ColorFilterBlock.GetLabelText()`가 `StackLabelFormat.ByRatio()` 대신 `ByValue()`("R G B", 공백 구분)를 쓰도록 변경. `ByRatio()`는 현재 호출하는 곳이 없어졌지만(비율로 판정하는 기물이 새로 생기면 쓸 수 있어 삭제하지 않고 남겨둠).
- [x] **색상별 `max`를 공용 상수 하나로 통합** (3.3) — `Config`에서 `max` 필드를 없애고(`start`만 남김), 클래스 전체가 공유하는 `public const int StackMax = 15`를 새로 추가. R/G/B가 각자 다른 상한을 가질 수 있던 구조를 없애 "일부 색상만 값을 깜빡하고 다르게 설정" 하는 종류의 버그(방금 겪은 것) 자체가 발생할 수 없게 함 — 상한을 바꾸려면 이제 `StackMax` 상수 한 곳만 고치면 된다. Player 프리팹의 저장된 `max` 값들도 정리(제거).
...
- [x] **조준+좌클릭 상호작용 (기획서 미기재, 신규)** — 걸어서 닿는 것과 별개로, 카메라 정면 1.3칸 이내에서 기물을 조준하고 좌클릭하면 상호작용 발동. `IInteractable`(`TryInteract()`) 인터페이스를 `AcquireObjectBase`/`ClearObjectBase`/`ConsumableObjectBase`가 구현(필터는 제외 — 마커 시스템과 동일한 대상). 기존에 걸어서 닿으면 발동하던 `OnTriggerEnter`는 제거해, 이제 이 세 종류(팔레트/캔버스/스택 체인저·컬러 체인저·지우개)는 조준+좌클릭으로만 발동한다(필터는 여전히 걸어서 통과하는 방식 그대로). `Player/InteractionController.cs` 신규 — 매 프레임 카메라 정면으로 1.3칸 레이캐스트(`QueryTriggerInteraction.Collide`)를 쏴서 가장 가까운 대상 하나만 판정(벽·닫힌 필터가 앞에 있으면 자연히 막힘, 별도 가림 판정 불필요), `InputManager.ReadInteract()`(좌클릭)로 발동. 강조 표시는 `MapObjectBase.SetHighlighted(bool)`(`highlightRoot` 자식 오브젝트를 켜고 끄기만 함, 라벨과 동일한 "미리 배치해둔 자식 재사용" 패턴)로 처리하며, 시각 효과는 새 셰이더 `Shaders/InteractionHighlightOutline.shader`(인버티드 헐 아웃라인)를 쓰는 별도 메시(원본과 같은 메시 참조)로 구현한다. 조준 대상 유무는 `InteractableTargetChanged` 이벤트로 알리고, `UI/CrosshairController.cs`가 이를 구독해 조준점 색을 바꾼다. 아웃라인 머티리얼(`Outline.mat`, `Custom/InteractionHighlightOutline` 셰이더 적용) 생성 완료, 기물 5종(팔레트/스택 체인저/컬러 체인저/지우개/캔버스) 프리팹 모두 `Outline` 하이라이트 오브젝트 적용 + `highlightRoot` 연결 완료, `InteractionController` Player 부착 완료, `CrosshairController` 연결 완료 — 에디터 작업까지 전부 마무리됨.
...
- [x] **HUD 색상 스와치 → 뷰모델 붓 색상으로 대체** (3.4) — `ColorStackHUD.colorSwatch`를 따로 연결하는 대신, 1인칭 뷰모델 붓(`BrushViewmodel`, 아래 참고)의 붓 끝 색이 플레이어 현재 RGB 스택 색을 보여주는 역할을 겸하기로 함. 별도 스와치 인스펙터 연결은 더 이상 필요 없음.
